...
class GitConfig:
    ...
    def get_ssh_private_key(self):
        return self.config.get('ssh_private_key', '')
    
    def is_configured(self):
        return bool(self.get_name() and self.get_email() and self.get_ssh_key())
    
    def set_config(self, name, email, ssh_key, ssh_private_key='', git_token=''):
        self.config['name'] = name
        self.config['email'] = email
        self.config['ssh_key'] = ssh_key
        if ssh_private_key:
            self.config['ssh_private_key'] = ssh_private_key
        if git_token:
...
class GitSetupWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent):
        ...
        self.ssh_key_display.insert("1.0", "SSH public key will appear here after generation...")
        ...
        step4_label = ctk.CTkLabel(
            content,
            text="Step 4: Setup SSH Agent",
            font=ctk.CTkFont(family="Roboto", size=15, weight="bold"),
            text_color=COLORS['text_primary']
        )
        step4_label.pack(anchor="w", pady=(15, 10))
        
        self.setup_agent_btn = SmoothButton(
            content,
            text="Setup SSH Agent",
            width=180,
            height=35,
            fg_color=COLORS['accent_blue'],
            command=self.setup_ssh_agent
        )
        self.setup_agent_btn.pack(side="left", pady=5)
        ...
        self.agent_status.pack(side="left", padx=(15, 0), pady=5)
    ...
    def load_saved_config(self):
        ...
        if config.get('ssh_key'):
            ...
            self.ssh_key_display.configure(state="disabled")
            self.agent_status.configure(text="SSH Agent setup completed", text_color=COLORS['accent_green'])
            ...
            try:
                ...
                if os.path.exists(key_path + ".pub"):
                    ...
                    with open(key_path, 'r') as f:
                        self.ssh_private_key = f.read().strip()
                    self.ssh_generated = True
    ...
    def setup_ssh_agent(self):
        ...
        self.setup_agent_btn.configure(state="disabled", text="Setting up...")
        self.agent_status.configure(text="Setting up SSH agent...", text_color=COLORS['accent_orange'])
        
        def setup():
            try:
                # Start SSH agent
                subprocess.run("eval $(ssh-agent -s)", shell=True, capture_output=True, text=True)
                ...
                if os.path.exists(ssh_key_path):
                    ...
                    if result.returncode == 0:
                        self.agent_status.configure(text="SSH Agent: Key added successfully!", text_color=COLORS['accent_green'])
                        # Test connection after agent setup
                        self.test_ssh_connection()
                    else:
                        self.agent_status.configure(text=f"SSH Agent: {result.stderr[:50]}", text_color=COLORS['accent_red'])
                else:
                    self.agent_status.configure(text="SSH Agent: Key file not found", text_color=COLORS['accent_red'])
                
                self.setup_agent_btn.configure(state="normal", text="Setup SSH Agent")
                
            except Exception as e:
                self.agent_status.configure(text=f"Error: {str(e)[:50]}", text_color=COLORS['accent_red'])
                self.setup_agent_btn.configure(state="normal", text="Setup SSH Agent")
    ...
    def copy_ssh_key(self):
        if self.ssh_key:
            ...
            messagebox.showinfo("Copied", "SSH public key copied to clipboard!")
...
class GitPushWindow(ctk.CTkToplevel):
    """Window yo gukora Git Push - all inside app using SSH"""
    ...
    def execute_push(self):
        ...
        def push():
            try:
                ...
                # Setup SSH agent before push
                self.safe_update_ui(self.push_status, text="Setting up SSH agent...", text_color=COLORS['accent_orange'])
                self.push_progress.set(0.1)
                self.output_text.insert("end", "[0] Setting up SSH agent...\n")
                ...
                ssh_key_path = os.path.join(os.path.expanduser("~"), ".ssh", "id_rsa")
                if os.path.exists(ssh_key_path):
                    subprocess.run(f'ssh-add "{ssh_key_path}"', shell=True, capture_output=True, text=True)
                    self.output_text.insert("end", "    SSH key added to agent\n")
                else:
                    self.output_text.insert("end", "    Warning: SSH key not found\n")
